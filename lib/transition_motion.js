(function (root, factory) {
  ...
  function getCyberMosaicQuality(mode = "preview", frame = {}) {
    const value = String(mode || "preview").toLowerCase();
    if (value === "palette" || value === "thumbnail") {
      return { qualityMode: "palette", maxTiles: 56, maskMaxWidth: 220, frameBuckets: 36, border: false };
    }
    if (value === "render" || value === "final") {
      return { qualityMode: "render", maxTiles: 220, maskMaxWidth: 520, frameBuckets: 96, border: false };
    }
    const width = Math.max(1, Number(frame.width || frame.frameW || 0));
    const maxTiles = width > 1280 ? 150 : 128;
    return { qualityMode: "preview", maxTiles, maskMaxWidth: 360, frameBuckets: 60, border: false };
  }

  function normalizeCyberMosaicOptions(raw = {}) {
    const reducedMotion = !!raw.reducedMotion;
    const quality = getCyberMosaicQuality(raw.qualityMode || raw.previewMode || raw.renderMode || CYBER_DEFAULTS.qualityMode, raw);
    const normalized = {
      ...
      maxTiles: Math.round(clamp(Number(raw.maxTiles ?? quality.maxTiles), 24, 260)),
      maskMaxWidth: Math.round(clamp(Number(raw.maskMaxWidth ?? quality.maskMaxWidth), 96, 640)),
      frameBuckets: Math.round(clamp(Number(raw.frameBuckets ?? quality.frameBuckets), 20, 120)),
      border: false
    };
    ...
    return {
      ...
      maxTiles: Math.min(normalized.maxTiles, 56),
      maskMaxWidth: Math.min(normalized.maskMaxWidth, 220),
      frameBuckets: Math.min(normalized.frameBuckets, 32)
    };
  }
  ...
  function buildCyberMosaicRectSet(options = {}) {
    ...
    const totalRects = Math.max(24, Math.min(cfg.maxTiles, Math.round((cfg.maxTiles * 0.42) + (cfg.maxTiles * 0.58 * cfg.tileDensity))));
  ...
  function buildCyberMaskCacheKey(progress, frameW, frameH, options = {}) {
    const cfg = normalizeCyberMosaicOptions(options);
    const width = Math.max(96, Math.round(Math.min(cfg.maskMaxWidth, Math.max(96, Number(frameW || 1)))));
    const height = Math.max(54, Math.round(width * (Math.max(1, Number(frameH || 1)) / Math.max(1, Number(frameW || 1)))));
    const frameBucket = Math.round(clamp(progress, 0, 1) * cfg.frameBuckets);
  ...
  function renderCyberMosaicMaskCanvas(progress, frameW, frameH, options = {}) {
    ...
    const eased = cyberMosaicEase(progress, cfg);
    const settle = smoothstep(getCyberSettleStart(cfg), 1, eased);
    const tileBoost = getCyberTileBoost(cfg);
    ...
    for (let y = 0; y < height; y += 1) {
      ...
      for (let x = 0; x < width; x += 1) {
        ...
        for (let i = 0; i < rects.length; i += 1) {
          const rect = rects[i];
          if (!isPointInsideRect(px, py, rect)) continue;
          const localReveal = smoothstep(rect.revealAt - rect.revealSoftness, rect.revealAt + rect.revealSoftness, eased);
          if (localReveal <= 0.0001) continue;
          tileReveal = Math.max(tileReveal, localReveal);
        }
        const reveal = clamp(Math.max(settle, tileReveal * tileBoost), 0, 1);
        const alpha = Math.round((cfg.mode === "in" ? reveal : (1 - reveal)) * 255);
        data[cursor] = 255;
        data[cursor + 1] = 255;
        data[cursor + 2] = 255;
        data[cursor + 3] = alpha;
