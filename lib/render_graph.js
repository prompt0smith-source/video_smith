(function (root, factory) {
  ...
  function buildCrossSectionBoundaries(project, startIdx = 0) {
    const clips = sortVideoClips(project);
    const boundaries = [];
    const seen = new Set();
    let boundaryIdx = Math.max(0, Number(startIdx || 0));
    for (const fromClip of clips) {
      const fromSectionId = Math.max(1, Math.round(toNumber(fromClip?.section, 1)));
      const fromEnd = clipEnd(fromClip);
      for (const toClip of clips) {
        if (!fromClip || !toClip || fromClip === toClip) continue;
        const toSectionId = Math.max(1, Math.round(toNumber(toClip?.section, 1)));
        if (fromSectionId === toSectionId) continue;
        const gap = clipStart(toClip) - fromEnd;
        if (Math.abs(gap) > GAP_EPSILON + EPSILON) continue;
        const key = `${fromClip.id || ""}->${toClip.id || ""}`;
        ...
        const section = Math.min(fromSectionId, toSectionId);
        boundaries.push({
          idx: boundaryIdx++,
          section,
          orderInSection: Number.MAX_SAFE_INTEGER,
          crossSection: true,
          fromSectionId,
          toSectionId,
          trackId: `video:${section}`,
          time: clipStart(toClip),
          fromClip,
          toClip,
          fromClipId: fromClip?.id,
          ...
          overlapEnd,
          overlapDuration,
          transitionEligible: overlapDuration <= GAP_EPSILON + EPSILON && gapDuration <= GAP_EPSILON + EPSILON
        });
      }
    }
    return boundaries.sort((a, b) => (
      a.time - b.time
      || a.section - b.section
      || a.fromSectionId - b.fromSectionId
      || a.toSectionId - b.toSectionId
      || String(a.fromClipId || "").localeCompare(String(b.fromClipId || ""))
    ));
  }
  ...
  function normalizeProjectTransitions(project = {}) {
    ...
    const analysis = ensureAnalysis(next);
    ...
    const preferBoundary = (previous, candidate) => {
      if (!previous) return candidate;
      if (!!previous.crossSection !== !!candidate.crossSection) return previous.crossSection ? candidate : previous;
      const prevGap = Math.abs(toNumber(previous.gapDuration, 0));
      const nextGap = Math.abs(toNumber(candidate.gapDuration, 0));
      return nextGap < prevGap ? candidate : previous;
    };
    const byFrom = new Map();
    const byTo = new Map();
    boundaries.forEach((boundary) => {
      byFrom.set(boundary.fromClipId, preferBoundary(byFrom.get(boundary.fromClipId), boundary));
      byTo.set(boundary.toClipId, preferBoundary(byTo.get(boundary.toClipId), boundary));
    });
    ...
    const addSeam = (raw, boundary, source = "explicit") => {
      ...
      const seam = {
        ...
        fromSectionId: Math.max(1, toNumber(boundary.fromSectionId ?? boundary.fromClip?.section, boundary.section)),
        toSectionId: Math.max(1, toNumber(boundary.toSectionId ?? boundary.toClip?.section, boundary.section)),
  ...
  function ensureAnalysis(target) {
    ...
    const baseSections = buildSections(project);
    const sameSectionBoundaries = baseSections.flatMap((section) => section.boundaries);
    const crossSectionBoundaries = buildCrossSectionBoundaries(project, sameSectionBoundaries.length);
    const sections = baseSections.map((section) => ({
      ...section,
      boundaries: [...section.boundaries]
    }));
    const sectionMap = new Map(sections.map((section) => [section.section, section]));
    crossSectionBoundaries.forEach((boundary) => {
      const section = sectionMap.get(boundary.section);
      if (!section) return;
      section.boundaries.push(boundary);
      section.boundaries.sort((a, b) => (
        a.time - b.time
        || (Number(!!b.crossSection) - Number(!!a.crossSection))
        || a.idx - b.idx
      ));
    });
    const boundaries = [...sameSectionBoundaries, ...crossSectionBoundaries];
  ...
  function matchBoundary(boundary, raw, keyInfo) {
    ...
    if (raw.fromClipId && raw.toClipId) {
      const sectionMatch = (!raw.fromSectionId || Math.max(1, Math.round(toNumber(raw.fromSectionId, 1))) === Math.max(1, Math.round(toNumber(boundary.fromSectionId ?? boundary.fromClip?.section, boundary.section))))
        && (!raw.toSectionId || Math.max(1, Math.round(toNumber(raw.toSectionId, 1))) === Math.max(1, Math.round(toNumber(boundary.toSectionId ?? boundary.toClip?.section, boundary.section))));
      return sectionMatch
  ...
  function findTransitionSeamCandidates(input = {}) {
    const project = input.project || input;
    const analysis = ensureAnalysis(project);
    const time = toNumber(input.time, NaN);
    if (!Number.isFinite(time)) return [];
    const targetSectionId = input.targetSectionId == null
      ? null
      : Math.max(1, Math.round(toNumber(input.targetSectionId, 1)));
    const sourceClipId = String(input.sourceClipId || "");
    const toleranceSec = clamp(toNumber(input.toleranceSec, 0.08), 0.01, 0.35);
    return (analysis.boundaries || [])
      .filter((boundary) => boundary.transitionEligible)
      .map((boundary) => {
        const fromSectionId = Math.max(1, Math.round(toNumber(boundary.fromSectionId ?? boundary.fromClip?.section, boundary.section)));
        const toSectionId = Math.max(1, Math.round(toNumber(boundary.toSectionId ?? boundary.toClip?.section, boundary.section)));
        const distance = Math.abs(toNumber(boundary.time, 0) - time);
        let score = distance;
        if (targetSectionId != null) {
          if (fromSectionId === targetSectionId && toSectionId === targetSectionId) score -= 0.025;
          else if (fromSectionId === targetSectionId || toSectionId === targetSectionId || Math.round(toNumber(boundary.section, 1)) === targetSectionId) score -= 0.012;
          if (boundary.crossSection) score += 0.006;
        }
        if (sourceClipId && (boundary.fromClipId === sourceClipId || boundary.toClipId === sourceClipId)) score -= 0.02;
        return {
          ...boundary,
          fromSectionId,
          toSectionId,
      ...
      .filter((boundary) => boundary.distance <= toleranceSec + EPSILON)
      .sort((a, b) => a.score - b.score || a.distance - b.distance || a.idx - b.idx);
  }
